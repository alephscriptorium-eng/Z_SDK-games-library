/**
 * Config MCP solve-coagula: una instancia = UN actor.
 */
...
export function getServerConfig(env = process.env, overrides = {}) {
  const actor = overrides.actor || env.ZEUS_SOLVE_PLAYER_ACTOR || DEFAULT_ACTOR;
  const endpoints = resolveSolveEndpoints(env);
  return {
    actor,
    name: `solve-player-mcp-${actor}`,
    port: endpoints.mcpPort,
    host: resolveZeusHost(),
    room: env.ZEUS_SOLVE_ROOM || DEFAULT_SOLVE_ROOM,
    confirmTimeoutMs: Number(env.ZEUS_SOLVE_MCP_TIMEOUT_MS || DEFAULT_CONFIRM_TIMEOUT_MS),
    noopMs: Number(env.ZEUS_SOLVE_MCP_NOOP_MS || DEFAULT_NOOP_MS)
  };
}
